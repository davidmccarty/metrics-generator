 ...
 *    node="ip-100-79-2-106.eu-central-1.compute.internal",
 *    pod="klusterlet-addon-workmgr-5c7f4946bf-z2gv6",
...
class Metric {
  constructor(node, container, pod, val) {
    ...
    this.metric = new prom.Gauge({
      ...
      help: "mock kube_pod_container_resource_limits_memory_bytes",
...
new Metric("node-1", "container-1", "pod-1", 50).start();
new Metric("node-2", "container-1", "pod-1", 55).start();
new Metric("node-3", "container-1", "pod-1", 60).start();
new Metric("node-4", "container-1", "pod-1", 65).start();
new Metric("node-5", "container-1", "pod-1", 70).start();
new Metric("node-6", "container-1", "pod-1", 75).start();
