 ...
 *    namespace="maint5-eu-central-1-aws-ats-openpaas-cc",
 *    node="ip-100-79-2-90.eu-central-1.compute.internal",
 *    pod="search-prod-124c9-search-aggregator-5cf6db597c-cqdkl",
...
class Metric {
  constructor(node, container, pod, val) {
    ...
    this.metric = new prom.Gauge({
      ...
      help: "mock kube_pod_container_resource_requests_cpu_cores",
...
new Metric("node-1", "container-1", "pod-1", 0.30).start();
new Metric("node-2", "container-1", "pod-1", 0.35).start();
new Metric("node-3", "container-1", "pod-1", 0.40).start();
new Metric("node-4", "container-1", "pod-1", 0.45).start();
new Metric("node-5", "container-1", "pod-1", 0.50).start();
new Metric("node-6", "container-1", "pod-1", 0.55).start();
